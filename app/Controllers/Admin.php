...
//        'username_data'=>$userData
    ...
    public function widthrawalEdit($id)
    {
        ...
        $userdetails =  $db->query("Select * FROM users WHERE id={$user_id}");
        $userDetails= $userdetails->getResult();
        $updatedEarnings = $userDetails[0]->earnings - $results[0]->amount;
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($this->validate($rules)) {
                ...
                $query = $db->query("UPDATE users SET earnings=$updatedEarnings, payouts=$payoutAmount WHERE id={$user_id}");
                $session = session();
                $session->setFlashdata('msg', 'Account is updated successfully');
